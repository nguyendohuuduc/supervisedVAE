...
train = np.loadtxt(open("50-dim-z-one-hot-labelmnist-train.csv", "rb"), delimiter=",", skiprows=0)
trainX = train[:,:-10]
trainYone_hot = train[:,-10:].astype(np.int32)
test = np.loadtxt(open("50-dim-z-one-hot-labelmnist-test.csv", "rb"), delimiter=",", skiprows=0)
testX = test[:,:-10]
...
def SVM():
    model = svm.SVC(kernel="rbf",decision_function_shape='ovo',random_state=0,max_iter=100000)
