...
def parse_args():
    ...
    parser.add_argument('--dim_z', type=int, default='50', help='Dimension of latent vector', required = True)
...
def main(args):
    ...
    # network architecture
    x_, z, loss, neg_marginal_likelihood, KL_divergence = vae.autoencoder(x_hat, x,dim_img, dim_z, n_hidden, keep_prob)
        ...
        for epoch in range(n_epochs):
            ...
            if epoch == n_epochs-1:
                for i in range(total_batch):
                    ...
                    batch_xs_target = batch_xs_input
                    my_z=sess.run(z,feed_dict={x_hat: batch_xs_input, x: batch_xs_target, keep_prob : 1.0})
                    my_z=tf.concat([my_z,tf.cast(batch_ys_input,dtype=tf.float64)],axis=1)
                    train_z.append(my_z)
                ...
                for curr_iter in range(test_iters):
                    ...
                    my_z=tf.concat([my_z,tf.cast(batch_ys_input,dtype=tf.float64)],axis=1)
                    test_z.append(my_z)
                Train_z =tf.concat(train_z,axis=0).eval()
                Test_z = tf.concat(test_z,axis=0).eval()
                print(Train_z.shape)
                print(Test_z.shape)
                np.savetxt("%0.2d-dim-z-one-hot-labelmnist-train.csv" %(dim_z),Train_z,delimiter=',')
                np.savetxt("%0.2d-dim-z-one-hot-labelmnist-test.csv" %(dim_z),Test_z,delimiter=',')
            ...
            if min_tot_loss > tot_loss or epoch+1 == n_epochs:
                ...
                # Plot for reproduce performance
                if PRR:
                    x__PRR = sess.run(x_, feed_dict={x_hat: x_PRR,keep_prob : 1})
                    x__PRR_img = x__PRR.reshape(PRR.n_tot_imgs, IMAGE_SIZE_MNIST, IMAGE_SIZE_MNIST)
                    PRR.save_images(x__PRR_img, name="/vae_PRR_epoch_%02d_dimz_%02d" %(epoch,dim_z) + ".jpg")
