...
def gaussian_MLP_conditional_encoder(x, n_hidden, n_output, keep_prob):
...
def bernoulli_MLP_conditional_decoder(z,n_hidden, n_output, keep_prob, reuse=False):

    with tf.variable_scope("bernoulli_MLP_decoder", reuse=reuse):
        # concatenate condition and latent vectors
        input = tf.concat(axis=1, values=[z])
...
def autoencoder(x_hat, x,dim_img, dim_z, n_hidden, keep_prob):
    ...
    mu, sigma = gaussian_MLP_conditional_encoder(x_hat,n_hidden, dim_z, keep_prob)

    # sampling by re-parameterization technique
    z = mu + sigma * tf.random_normal(tf.shape(mu), 0, 1, dtype=tf.float32)

    # decoding
    x_ = bernoulli_MLP_conditional_decoder(z, n_hidden, dim_img, keep_prob)
...
def decoder(z, dim_img, n_hidden):
    ...
    x_ = bernoulli_MLP_conditional_decoder(z, n_hidden, dim_img, 1.0, reuse=True)
